...
public class GSIConfigTest {
    ...
    public void testGettersAndSetters() {
        ...
        assertEquals(1d, profile.getBufferPeriod().get(), 0.001d);
    
        profile.setTimeoutPeriod(2d);
        assertEquals(2d, profile.getTimeoutPeriod().get(), 0.001d);
    
        profile.setHeartbeatPeriod(3d);
        assertEquals(3d, profile.getHeartbeatPeriod().get(), 0.001d);
    
        profile.setThrottlePeriod(4d);
        assertEquals(4d, profile.getThrottlePeriod().get(), 0.001d);
    
        profile.setPrecisionPosition(421);
        assertEquals((Integer)421, profile.getPrecisionPosition().get());
    
        profile.setPrecisionTime(422);
        assertEquals((Integer)422, profile.getPrecisionTime().get());
